...
API_TOKEN = '1460207876:AAHoLQS_SBurKWAOoiQABhn3_Jmfw7I_h4s'
TESTING = True
DEPLOY = not TESTING
...
@@ -55,10 +71,11 @@ def counter_inc(self):
        self.counter += 1
    ...
    def increment_used(self):
        for l in self.langs:
@@ -68,11 +85,13 @@ def increment_used(self):
                self.used[l] = 0
...
@@ -85,44 +104,54 @@ def correct_used(self):
            del self.used[lang]
...
@@ -145,32 +174,37 @@ def send_message_global(message):
...
def send_text_group_better(message):
    k = chats[message.chat.id].get_count_of_langs()
@@ -180,25 +214,27 @@ def send_text_group_better(message):
    chats[message.chat.id].increment_used()
...
def send_text(message):
    ...
    if txt == show_it:
@@ -207,33 +243,42 @@ def send_text(message):
        choice(message)
